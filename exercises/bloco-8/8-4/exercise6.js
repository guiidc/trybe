...
function studentAverage() {
  ...
  let averageGrades = grades.map((v) => v.reduce((pv, cv) => (pv + cv)) / v.length);
  let studentsAverageArray = students.map((v, i) => ({name: v, average: averageGrades[i]}));
  return studentsAverageArray;
}
